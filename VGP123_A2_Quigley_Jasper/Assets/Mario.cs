...
public class Mario : MonoBehaviour {
	...
	void Update() {
		...
		keyD = Input.GetKey(KeyCode.RightArrow);
		keyA = Input.GetKey(KeyCode.LeftArrow);
...
//		if (Input.GetKeyDown(KeyCode.Space)) print(grounded);
	...
	private void Move(Vector2 move) {
		...
		Vector2 attemptPos = curPos + move+d;
	...
	Vector2 HandleCollisions(Vector2 move, CollisionInfo[] collisions) {
		...
		print("Handling Collisions: "+collisions.Length +"  :  "+ move);
		...
		foreach (CollisionInfo collision in collisions) {
			...
			if (collision.hitRight) {
				...
				if (collision.obj.name == "Sizer" && !poweredUp)
				{
					poweredUp = true;
					Destroy(Sizer.gameObject);
					StartCoroutine(StartSizing(true));
				}
	...
	private IEnumerator StartSizing(bool kill) {
		float waitTime = 0.2f;
		dimensions = new Vector2(1, 2);
	...
	void HitTopBlock(CollisionInfo collision) {
		if (collision.obj.blockType == BlockType.breakable) {
			...
			if (!animator.GetBool("used")) {
				...
				collision.obj.StartBounce();
				if(collision.obj.name == "coinSizer")
                {
